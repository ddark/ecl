...
public:
    ...
    void LoadDataFromDataBase()
    {
        ...
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, "Loading AutoLearn...");
        ...
        do
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "AutoLearn: Spell (ID: %u) non-existing", Spell.SpellId);
                continue;
            }
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "AutoLearn: Spell (ID: %u) RequiredClassMask (Mask: %u) non-existing", Spell.SpellId, Spell.RequiredClassMask);
                continue;
            }

            if (Spell.RequiredRaceMask != 0 && !(Spell.RequiredRaceMask & RACEMASK_ALL_PLAYABLE))
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "AutoLearn: Spell (ID: %u) RequiredRaceMask (Mask: %u) non-existing", Spell.SpellId, Spell.RequiredRaceMask);
                continue;
            }

            if (Spell.RequiredSpellId != 0 && !sSpellMgr->GetSpellInfo(Spell.RequiredSpellId))
            {
                TC_LOG_ERROR(LOG_FILTER_SQL, "AutoLearn: Spell (ID: %u) RequiredSpellId (ID: %u) non-existing", Spell.SpellId, Spell.RequiredSpellId);
                continue;
            }
        ...
        TC_LOG_INFO(LOG_FILTER_SERVER_LOADING, ">> Loaded %u spells for AutoLearn in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    }
};
